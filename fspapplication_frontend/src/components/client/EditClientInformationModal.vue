...
const initForm = () => {
  if (props.clientData) {
    editForm.name = props.clientData.name || '';
    editForm.phone = props.clientData.phone || '';
    editForm.email = props.clientData.email || '';
    
    // Strip https:// from website for display in input
    if (props.clientData.website) {
      editForm.websiteInput = props.clientData.website.replace(/^https?:\/\//, '');
    } else {
      editForm.websiteInput = '';
    }
    
    editForm.abn = props.clientData.abn || '';
  }
};
...
// Initialize on mount and when clientData changes
onMounted(initForm);
