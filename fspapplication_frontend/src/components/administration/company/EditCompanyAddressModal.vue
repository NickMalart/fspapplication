...
const props = defineProps<{
  companyData: ExtendedCompanyProfile;
  isSaving: boolean;
}>();
...
const formData = ref<Partial<ExtendedCompanyProfile>>({
  ...
  streetName: props.companyData.streetName || '',
  suburb: props.companyData.suburb || '',
  ...
  latitude: props.companyData.latitude,
...
const saveChanges = () => {
  ...
  // Convert all empty strings to null (backend expects null, not empty strings)
  ...
  if (dataToSave.latitude !== null && dataToSave.latitude !== undefined) {
    // Ensure it's a number and has at most 6 decimal places
    ...
    if (!isNaN(dataToSave.latitude)) {
      // Limit to 9 total digits with 6 decimal places
...
// Watch for changes in companyData
watch(() => props.companyData, initForm, { immediate: true });
</script> 