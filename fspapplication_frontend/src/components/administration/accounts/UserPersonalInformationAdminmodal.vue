      ...
      <form @submit.prevent="saveChanges" class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          <input
            ...
            type="email"
            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 cursor-not-allowed opacity-70"
...
import { reactive, defineEmits, defineProps, computed, onMounted, watch, ref } from 'vue';
...
// Function to focus and open the date picker when calendar icon is clicked
...
const initForm = () => {
  if (props.userData) {
    ...
    editForm.phoneNumber = props.userData.profile?.phoneNumber || '';
    
    // Format date if it exists
    if (props.userData.profile?.dateOfBirth) {
      try {
        const date = new Date(props.userData.profile.dateOfBirth);
