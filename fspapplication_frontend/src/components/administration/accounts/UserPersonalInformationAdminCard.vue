<template>
  <ComponentCard title="Personal Information">
    ...
    <div v-else class="relative">
      ...
      <div v-if="userData" class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
        <div class="space-y-2">
        ...
        <div class="space-y-2">
          ...
          <p class="font-medium text-black dark:text-white">
            {{ userData.profile?.phoneNumber || 'Not provided' }}
          </p>
        </div>
        <div class="space-y-2">
    ...
    <UserPersonalInformationAdminModal
      v-if="isModalOpen && userData"
      :key="modalKey"
      :userData="userData"
      :isSaving="isSaving"
...
const handleSave = async (formData: { firstName: string, lastName: string, phoneNumber: string | null, dateOfBirth: string | null }) => {
  ...
  try {
    ...
    const result = await userProfileAdminStore.updateUserProfile(userId, {
    ...
    // Emit update event with updated user data
